...
CGDisplayCount getOnlineDisplayInfos(DisplayInfos* displayInfos) {
    ...
    for (int i = 0; i < (int)screenCount && validDisplayCount < MAX_DISPLAYS; i++) {
        ...
        // The built-in MacBook panel is not a DDC/CI target. Treating it as an
        // external display creates a false "Unknown Display" entry.
        if (CGDisplayIsBuiltin(currDisplay->id)) {
            continue;
        }
